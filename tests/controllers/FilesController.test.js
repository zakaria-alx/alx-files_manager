...
describe("+ FilesController", () => {
    ...
    const mockUser = {
        email: "alexcross@example.com",
        password: "alexcross665",
    };
    /**
     * 3 sample files
     * + 1 -> file
    ...
    const mockFiles = [
        ...
        },
        {
            name: "movies.txt",
            type: "file",
            data: ["+ Inception", "+ Iron Man", "+ Extraction", ""].join("\n"),
            b64Data() {
                return Buffer.from(this.data, "utf-8").toString("base64");
            },
        },
        {
            name: "48_laws_of_power.md",
            type: "file",
            data: [
    ...
    const emptyCollections = (callback) => {
    ...
    before(function (done) {
        this.timeout(10000);
        emptyCollections(() => signUp(mockUser, () => signIn(mockUser, done)));
        emptyFolder(baseDir);
    });
    ...
    describe("+ POST: /files", () => {
        ...
        it("+ Fails if type is available but unrecognized", function (done) {
            ...
            request
                .post("/files")
                .set("X-Token", token)
                .send({ name: "movies.txt", type: "video" })
                .expect(400)
                .end((requestErr, res) => {
                    if (requestErr) {
                        return done(requestErr);
        ...
        it("+ Passes for valid values of a file", function (done) {
            this.timeout(5000);
            request
                .post("/files")
                .set("X-Token", token)
        ...
        it("+ Passes for valid values of a folder", function (done) {
            this.timeout(5000);
            request
                .post("/files")
                .set("X-Token", token)
        ...
        it("+ Passes if parentId is set and is of a folder", function (done) {
            this.timeout(5000);
            request
                .post("/files")
                .set("X-Token", token)
    ...
    describe("+ GET: /files/:id", () => {
        ...
        it("+ Passes if file is linked to user", function (done) {
            this.timeout(5000);
            request
    ...
    describe("+ GET: /files", () => {
        ...
        it("+ Returns empty list for unknown parentId", function (done) {
            this.timeout(5000);
            request
                .get("/files?parentId=7a45f90b2c5c7a8d4e1a0b2d")
                .set("X-Token", token)
                .expect(200)
                .end((requestErr, res) => {
                    if (requestErr) {
    ...
    describe("+ GET: /files/:id/data", () => {
        ...
        it("+ Passes if the file is not public but requested by the owner", function (done) {
            request
                .get(`/files/${mockFiles[0].id}/data`)
                .set("X-Token", token)
                .expect(200)
        ...
        it("+ Passes if the file is not public but requested by the owner [alt]", function (done) {
            request
